...
namespace FeatureFlowFramework.Diagnostics
{
    ...
    public class DataFlowProbe<T1, T2> : IDataFlowSink
    {
        ...
        readonly Predicate<T1> filter;
        readonly Func<T1, T2> convert;
        readonly TimeSpan timeSliceSize;
        TimeSliceCounter currentTimeSlice;
        LazySlim<AsyncManualResetEvent> manualResetEvent;

        long counter;
        CountingRingBuffer<(DateTime timestamp, T2 message)> messageBuffer;
        CountingRingBuffer<DateTime> timestampBuffer;
        CountingRingBuffer<TimeSliceCounter> timeSliceCounterBuffer;
