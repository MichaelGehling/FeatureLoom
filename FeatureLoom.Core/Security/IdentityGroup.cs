    ...
    public class IdentityGroup
    {
        ...
        public static async Task UpdateCache()
        {
            using(cacheLock.Lock())
            {
                ...
                if (!(await reader.TryReadAllAsync<IdentityGroup>().ConfiguredAwait()).TryOut(out cache)) throw new Exception("Failed update cache from storage");
            }
        }        
        ...
        public static async Task<(bool, IdentityGroup)> TryGetGroupAsync(string groupId, bool forceUpdateCache = false)
        {
            using (var lockHandle = cacheLock.LockReadOnly())
            {
                ...
                {
                    var reader = Storage.GetReader(StorageCategory);
                    if (!(await reader.TryReadAsync<IdentityGroup>(groupId).ConfiguredAwait()).TryOut(out var group)) return (false, null);
                    cache[groupId] = group;
                    return (true, group);
                }
                else
        ...
        public async Task StoreAsync()
        {
            ...
            if (!await Storage.GetWriter(storageCategory).TryWriteAsync(groupId, this).ConfiguredAwait()) throw new Exception($"Failed writing group {groupId} to storage");
        }
        ...
        public async Task RemoveFromStorageAsync()
        {
            ...
            if (!await Storage.GetWriter(storageCategory).TryDeleteAsync(groupId).ConfiguredAwait()) throw new Exception($"Failed removing group {groupId} from storage");
        }
