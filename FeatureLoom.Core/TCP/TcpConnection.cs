...
namespace FeatureLoom.TCP
{
    public sealed class TcpConnection : IDisposable
    {
        ...
        public TcpConnection(TcpClient client, IGeneralMessageStreamReader reader, IGeneralMessageStreamWriter writer, bool useConnectionMetaDataForMessages, IStreamUpgrader streamUpgrader = null)
        {
            ...
            messageWriter = new ProcessingEndpoint<object>(async msg => await WriteMessage(msg).ConfiguredAwait());
        ...
        async Task StartReceiving()
        {
            while(IsConnected)
            {
                try
                {
                    object message = await reader.ReadMessage(stream, cts.Token).ConfiguredAwait();
                    if (message != null)
                    {
        ...
        async Task WriteMessage(object message)
        {
            ...
            try
            {                
                await writer.WriteMessage(message, stream, cts.Token).ConfiguredAwait();
            }
            catch (Exception e)
            {
