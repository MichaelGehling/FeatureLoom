    ...
    void CreateByteArrayWriter(CachedTypeWriter typeWriter)
    {
        ...
        Action<byte[], bool, ByteSegment> BuildWriter() => (typeInfo, useBase64) switch
        {
            (TypeInfoHandling.AddAllTypeInfo, true) => (item, _, _) => 
                { StartTypeInfoObject(typeWriter.preparedTypeInfo); writer.WriteBytesAsBase64(item); FinishTypeInfoObject(); },
            (TypeInfoHandling.AddAllTypeInfo, false) => (item, _, _) => 
                { StartTypeInfoObject(typeWriter.preparedTypeInfo); writer.WriteBytesAsArray(item); FinishTypeInfoObject(); },
            (TypeInfoHandling.AddDeviatingTypeInfo, true) => (item, dev, _) => 
                { if (dev) StartTypeInfoObject(typeWriter.preparedTypeInfo); writer.WriteBytesAsBase64(item); if (dev) FinishTypeInfoObject(); },
            (TypeInfoHandling.AddDeviatingTypeInfo, false) => (item, dev, _) => 
                { if (dev) StartTypeInfoObject(typeWriter.preparedTypeInfo); writer.WriteBytesAsArray(item); if (dev) FinishTypeInfoObject(); },
    ...
    void CreateByteSegmentWriter(CachedTypeWriter typeWriter, bool nullable)
    {
        ...
        Action configureWriter = (nullable, typeInfo, useBase64) switch
        {
            (false, TypeInfoHandling.AddAllTypeInfo, true) => () => typeWriter.SetItemWriter<ByteSegment>((item, _, _) =>
                { StartTypeInfoObject(typeWriter.preparedTypeInfo); writer.WriteBytesAsBase64(item); FinishTypeInfoObject(); }, false),
            (false, TypeInfoHandling.AddAllTypeInfo, false) => () => typeWriter.SetItemWriter<ByteSegment>((item, _, _) =>
                { StartTypeInfoObject(typeWriter.preparedTypeInfo); writer.WriteBytesAsArray(item); FinishTypeInfoObject(); }, false),
            (false, TypeInfoHandling.AddDeviatingTypeInfo, true) => () => typeWriter.SetItemWriter<ByteSegment>((item, dev, _) =>
                { if (dev) StartTypeInfoObject(typeWriter.preparedTypeInfo); writer.WriteBytesAsBase64(item); if (dev) FinishTypeInfoObject(); }, false),
            (false, TypeInfoHandling.AddDeviatingTypeInfo, false) => () => typeWriter.SetItemWriter<ByteSegment>((item, dev, _) =>
                { if (dev) StartTypeInfoObject(typeWriter.preparedTypeInfo); writer.WriteBytesAsArray(item); if (dev) FinishTypeInfoObject(); }, false),
            ...
            (true, TypeInfoHandling.AddAllTypeInfo, true) => () => typeWriter.SetItemWriter<ByteSegment?>((item, _, _) =>
                { StartTypeInfoObject(typeWriter.preparedTypeInfo); if (item.HasValue) writer.WriteBytesAsBase64(item.Value); else writer.WriteNullValue(); FinishTypeInfoObject(); }, false),
            (true, TypeInfoHandling.AddAllTypeInfo, false) => () => typeWriter.SetItemWriter<ByteSegment?>((item, _, _) =>
                { StartTypeInfoObject(typeWriter.preparedTypeInfo); if (item.HasValue) writer.WriteBytesAsArray(item.Value); else writer.WriteNullValue(); FinishTypeInfoObject(); }, false),
            (true, TypeInfoHandling.AddDeviatingTypeInfo, true) => () => typeWriter.SetItemWriter<ByteSegment?>((item, dev, _) =>
                { if (dev) StartTypeInfoObject(typeWriter.preparedTypeInfo); if (item.HasValue) writer.WriteBytesAsBase64(item.Value); else writer.WriteNullValue(); if (dev) FinishTypeInfoObject(); }, false),
            (true, TypeInfoHandling.AddDeviatingTypeInfo, false) => () => typeWriter.SetItemWriter<ByteSegment?>((item, dev, _) =>
                { if (dev) StartTypeInfoObject(typeWriter.preparedTypeInfo); if (item.HasValue) writer.WriteBytesAsArray(item.Value); else writer.WriteNullValue(); if (dev) FinishTypeInfoObject(); }, false),
